...
//TFile* fDY	= TFile::Open("inputFiles/all_DY_M50.root");
TFile* fTT	    = TFile::Open("inputFiles/all_TT.root");
TFile* fWJ	    = TFile::Open("inputFiles/all_WJets.root");
TFile *fSigMuMuZ= TFile::Open("inputFiles/all_ExLepMuMuZ_M250.root");
TFile *fSigEEZ  = TFile::Open("inputFiles/all_ExLepEEZ_M250.root");
TFile *fNonDYBkg= TFile::Open("all_NonDYBkg.root");

TString chDir = "baseLowMET";
class MyEtherStackHisto{
  public : 
	//get histogram from root file. Return empty hist, if the hist does not exist.
	TH1F* getHisto(TFile *inRootFile, TString chDir, TString baseDir, TString histDir, TString histName);
        //decorate histogram
        TH1F* decorateHisto(TH1F* hist, TString myTit, TString xTit, TString yTit);
	//function to stack histos
    	TH1F* stackHisto(TFile *inRootFile, TString chDir, TString baseDir, TString histDir, TString histName, int color, double scale, THStack* MuptStack, TH1F* hMC);
...
//--------------------------------------------//
TH1F*  MyEtherStackHisto:: getHisto(TFile *inRootFile, TString chDir, TString baseDir, TString histDir, TString histName){
  TH1F* hist;
...
TH1F* MyEtherStackHisto:: decorateHisto(TH1F* hist, TString myTit, TString xTit, TString yTit){
  hist->SetTitle(myTit);
...
TH1F *  MyEtherStackHisto:: stackHisto(TFile *inRootFile, TString chDir, TString baseDir, TString histDir, TString histName, int color, double scale, THStack* MuptStack, TH1F* hMC){
  TH1F* hist = getHisto(inRootFile, chDir, baseDir, histDir, histName);
  hist->Scale(scale);  
...
double MyEtherStackHisto:: errBandUp(int iBin, TH1F *hCentral, TH1F *hJESPlus, TH1F *hJERPlus, TH1F *bTagPlus){
...
double MyEtherStackHisto:: errBandDown(int iBin, TH1F *hCentral, TH1F *hJESMinus, TH1F *hJERMinus, TH1F *bTagMinus){
...
TGraphAsymmErrors * MyEtherStackHisto:: UNCGRAPH(TH1F *hCentral, TH1F *hJESPlus, TH1F *hJESMinus, TH1F *hJERPlus, TH1F *hJERMinus, TH1F *bTagPlus, TH1F *bTagMinus, bool isFullGraph = false, bool isRatioGraph = false){
...
TH1F*  MyEtherStackHisto:: addHistoForUnc(TString chDir, TString baseDir, TString histDir, TString histName){
...
TPaveText * MyEtherStackHisto:: paveText(double minX, double minY, double maxX, double maxY, int lineColor, int fillColor, int size, int style, int font ){
...
void MyEtherStackHisto::makeHistoPositive(TH1F* hist, bool setErrorZero = false){
...
double MyEtherStackHisto::getStatUnc(TH1F* hCentral, double sError = 0.0){
...
void MyEtherStackHisto::overlapHisto(TH1F *h1, TH1F *h2, bool isRatio, TString histDir, TString histName, TString xTitle, double xmin, double xmax){
  gStyle->SetOptStat(0);
...
vector<double> MyEtherStackHisto::getTransFactDY(TFile* fData, TFile* fTT, TFile* fWJ, TFile* fVV, TString chDir, TString baseDir, TString histDir, TString histName, TString xTitle, double xmin, double xmax){
  //RegionC = LowMET, Iso
...
vector<double> MyEtherStackHisto::getTransFactDY2(TFile* fData, TFile* fNonDYBkg, TString chDir, TString baseDir, TString histDir, TString histName, TString xTitle, double xmin, double xmax){
...
TH1F* MyEtherStackHisto:: getDataDrivenDY(TString chDir, TString baseDir, TString histDir, TString histName, double transFact=1.0, double errorTransFact = 0.0){
